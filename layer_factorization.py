...
for layer in model.bert.encoder.layer:
    layer.attention.self.query = LinearWrapper(layer.attention.self.query, rank=attention_rank)
    layer.attention.self.key = LinearWrapper(layer.attention.self.key, rank=attention_rank)
    layer.attention.self.value = LinearWrapper(layer.attention.self.value, rank=attention_rank)
    # layer.attention.output.dense = LinearWrapper(layer.attention.output.dense, rank=attention_rank)

print(f"Factorized attention parameter count: {sum(p.numel() for p in model.parameters()):,}")
